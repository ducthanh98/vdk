...
import (
	...
	"github.com/ducthanh98/vdk/codec"
	"github.com/ducthanh98/vdk/codec/aacparser"
	"github.com/ducthanh98/vdk/codec/h264parser"
	"github.com/ducthanh98/vdk/codec/h265parser"
	"github.com/ducthanh98/vdk/format/rtsp/sdp"
)
...
func (client *RTSPClient) startStream() {
	...
	reconnect := func() error {
		...
		if client.pURL.Scheme == "rtsps" {
			tlsConn := tls.Client(conn, &tls.Config{InsecureSkipVerify: client.options.InsecureSkipVerify, ServerName: client.pURL.Hostname()})
			err = tlsConn.Handshake()
			if err != nil {
				return err
			}
		...
		client.Println("RTSP Client: Reconnected successfully")
		return nil
	}
